...
contract DeferredEquityPlan {
    ...
    uint interval = 365 days; //set to "1 minutes" used for testing
    uint unlock_time = now + interval;
    ...
    function distribute() public {
        ...
        require(unlock_time < now, "Must wait longer for distribution");
        require(distributed_shares<total_shares, "You have already claimed all allocated shares");

        // @TODO: Add 365 days to the `unlock_time`
        unlock_time = unlock_time + interval;
        ...
        distributed_shares = (now-start_time)/(interval) * annual_distribution;
